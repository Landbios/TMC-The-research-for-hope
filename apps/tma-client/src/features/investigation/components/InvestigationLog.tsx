...
export function InvestigationLog() {
  const [evidences, setEvidences] = useState<GlobalEvidence[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedClue, setSelectedClue] = useState<GlobalEvidence | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchLog = async () => {
      ...
      if (!error && data) {
        setEvidences(data as unknown as GlobalEvidence[]);
      }
      setLoading(false);
    };
  ...
  return (
    <div className="h-full flex flex-col bg-black/40 backdrop-blur-sm border border-blue-500/10 overflow-hidden">
      <div className="p-4 border-b border-blue-500/20 bg-blue-950/10 flex justify-between items-center">
        <h2 className="font-mono text-sm font-bold text-blue-400 uppercase tracking-[0.2em]">Registro de Evidencias</h2>
        <span className="font-mono text-[9px] text-zinc-500 uppercase">Archivos vinculados: {evidences.length}</span>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {evidences.map((item) => (
              <button 
                ...
                className="flex gap-4 p-3 bg-blue-900/5 border border-blue-500/10 hover:border-blue-500/40 hover:bg-blue-500/10 transition-all text-left group"
              >
      ...
      {selectedClue && (
        <div className="absolute inset-0 z-50 bg-black/95 p-6 animate-fade-in flex flex-col">
           ...
           <div className="flex-1 flex flex-col md:flex-row gap-8 items-center justify-center max-w-4xl mx-auto w-full">
              <div className="w-64 h-64 md:w-80 md:h-80 bg-zinc-900 border-2 border-blue-500/30 relative overflow-hidden shadow-[0_0_40px_rgba(59,130,246,0.1)]">
                <NextImage 
              ...
              <div className="flex-1 space-y-6">
                <div>
                   <h4 className="font-mono text-[10px] text-blue-500 uppercase tracking-[0.3em] font-bold mb-2">Archivo Neural Desbloqueado</h4>
                   <h2 className="font-mono text-2xl text-blue-400 uppercase leading-none border-l-4 border-blue-600 pl-4">{selectedClue.evidence.title}</h2>
                </div>
                ...
                <div className="flex justify-between border-t border-zinc-800 pt-4 font-mono text-[10px] text-zinc-500 uppercase">
                  <span>Descubierto el: {new Date(selectedClue.created_at).toLocaleDateString()}</span>
